const Ship = (length, xcoord, ycoord, position) => {
  ...
  return { length, shipsCoordinatesArray, hits, hit, position, isSunk, sunkStatus};
};
...
const Gameboard = () => {
  ...
  const placeShip = (length, xcoord, ycoord, position) => {
    if ((position === "vertical") && ((ycoord + length) > 9)) {
      //
    } else if ((position === "horizonal") && ((xcoord + length)) > 9) {
      //
    } else {
  ...
  return { placeShip, receiveAttack, shipArray, SquaresHit, determineIfConcluded, areAllShipsSunk };
};
...
const CustomName = Player("David", true);
